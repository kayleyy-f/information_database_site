...
// 3) On load, always show full list first
document.addEventListener('DOMContentLoaded', () => {
  render(schools);

  // now try to hook up filters
  ...
  // if any filter element is missing, abort binding
  if (!(typeFilterEl && degreeFilterEl && sizeFilterEl && campusFilterEl && sizeValEl)) {
    console.warn('Filter controls not found; list is shown unfiltered.');
    return;
  }

  // update the size label
  const updateSizeLabel = () => {
    const val = parseInt(sizeFilterEl.value, 10);
    sizeValEl.textContent = val === parseInt(sizeFilterEl.max, 10)
      ? `${val}+`
      : val;
  };
  updateSizeLabel();
